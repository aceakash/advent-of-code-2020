...
def findTwoNumsThatAddUpTo(sum, fromIndex=0):
    ...
    while curr < len(nums):
        ...
        try:
            match = nums.index(remaining, curr+1)
            return [num, nums[match]]
        except ValueError:
            curr = curr + 1
...
def first():
    matches = findTwoNumsThatAddUpTo(2020)
...
def second():
    ...
    while curr < len(nums):
        ...
        matches = findTwoNumsThatAddUpTo(remaining, curr+1)

        if len(matches) == 0:
            curr = curr + 1
        else:            
            print(matches)
            return num * matches[0] * matches[1]
