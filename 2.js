...
function main() {
    const validPasswords = input.filter(isValid)
    console.log(validPasswords)
    console.log(validPasswords.length)
}

function isValid(inputLine) {
    const {
        firstCharacter,
        secondCharacter,
        letter,
        password
    } = getInputParams(inputLine)

    return (firstCharacter === letter && secondCharacter !== letter)
        || (firstCharacter !== letter && secondCharacter === letter)
}

function count(password, letter) {
    const matches = password.split('').filter(c => c === letter)
    return matches.length
}


function getInputParams(inputLine) {
    // 2-6 c: fcpwjqhcgtffzlbj
    const bySpaces = inputLine.split(' ')
    const [firstPosition, secondPosition] = bySpaces[0].split('-').map(x => Number.parseInt(x))

    
    const letter = bySpaces[1][0]

    const password = bySpaces[2]

    const [firstCharacter, secondCharacter] = [password[firstPosition-1], password[secondPosition-1]]
