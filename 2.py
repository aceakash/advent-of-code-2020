...
def parseInputLine(inputLine):
    (numsPart, letterPart, password) = inputLine.split(' ')
    [firstNum, secondNum] = [int(x) for x in numsPart.split('-')]
    letter = letterPart[0]
    return (firstNum, secondNum, letter, password)

def isValidByFrequency(minTimes, maxTimes, letter, password):
    letterFreq = password.count(letter)
    return letterFreq >= minTimes and letterFreq <= maxTimes

def first():
    validPasswords = [inputLine for inputLine in input if isValidByFrequency(*parseInputLine(inputLine))]
    return len(validPasswords)

def isValidByPosition(firstPosition, secondPosition, letter, password):    
    return  (password[firstPosition-1] == letter and password[secondPosition-1] != letter) \
        or  (password[firstPosition-1] != letter and password[secondPosition-1] == letter)

def second():
    validPasswords = [inputLine for inputLine in input if isValidByPosition(*parseInputLine(inputLine))]
    return len(validPasswords)
